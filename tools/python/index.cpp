...
encoding again.
  ...
  py::class_<rpg::Music> music(m, "Music");
  music.def("__repr__", [](const rpg::Music& mus) { return "<pylcf.Music \"" + mus.name + "\">"; });

  py::class_<rpg::MapInfo> mapinfo(m, "MapInfo");
  mapinfo.def("__repr__", [](const rpg::MapInfo& info) { return "<pylcf.MapInfo \"" + std::string(info.name) + "\">"; });

  py::class_<rpg::Event> event(m, "Event");
  event.def("__repr__", [](const rpg::Event& ev) { return "<pylcf.Event \"" + std::string(ev.name) + "\">"; });

  py::class_<rpg::Variable> variable(m, "Variable");
  variable.def("__repr__", [](const rpg::Variable& var) { return "<pylcf.Variable \"" + std::string(var.name) + "\">"; });

  py::class_<rpg::Switch> switch_(m, "Switch");
  variable.def("__repr__", [](const rpg::Switch& switch_) { return "<pylcf.Switch \"" + std::string(switch_.name) + "\">"; });

  DefineStructs<rpg::Encounter, rpg::EventPageCondition, rpg::EventPage>(m);
  DefineStructs(music, mapinfo, variable, switch_, event);
  ...
  DefineStructs<
    ...
    rpg::MoveRoute, rpg::Map,
    rpg::BattlerAnimationPose, rpg::BattlerAnimationWeapon, rpg::BattlerAnimation,
        ...
        for (std::string &candidate : candidates) {
          ...
          if (!StringIsAscii(system_name)) {
            ...
            if (std::filesystem::exists(system_path)) {
              encoding = parsed_encoding = system_path;
              break;
            }
          }
        }
